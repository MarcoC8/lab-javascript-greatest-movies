...
function scoresAverage(moviesArray) {
    ...
    const avgNum = moviesArray.reduce((x, y) => { 
        if (!y.score){
            return x;
        }     
    return x + y.score 
    }, 0)
    return  Number(parseFloat(avgNum / moviesArray.length).toFixed(2))
}
// Iteration 4: Drama movies - Get the average of Drama Movies
function dramaMoviesScore(moviesArray) {
    const filteredDrama = moviesArray.filter((movie)=> {
...
function orderAlphabetically(moviesArray) {
    const alphabeticalArray = [...moviesArray].map((movie) =>{
        return movie.title
    });
